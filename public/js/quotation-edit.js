$( document ).ready(function() {
	...
	$("#product-add").click(function(e){
	    ...
	    if($("#product-id").val() == '' ) {
		  ...
		  $.ajax({
		    type :  "POST",
		    url  :  burl + "invoice/aj_add_product_details",
		    data :  { 
	...
	$('.product-grid').on('click', '.edit-id', function(e) {
		...
		$.ajax({
		  type: "POST",
		  url  :  burl + "invoice/aj_get_product_details/"+row[1],
		  data: { },
		  success: function(data){ 
		  	// alert(data);
		    var result = $.parseJSON(data);
	...
	$("#product-update").click(function(e){
		...
		//Insert
		  ...
		  $.ajax({
		    type :  "POST",
		    url  :  burl + "invoice/aj_edit_product_details/"+editkey,
		    data :  { 
		    	quotation_id  	  : $("#hid-quo-id").val(),
		      	product_id     	  : $("#product-id").val(),
		      	unit_price        : $("#unit-price").val(),
	...
	$.ajax({
	  type :  "POST",
	  url  :  burl + "invoice/aj_remove_quotation_product/"+row[1],
	  data :  { },
	  success: function(data){  
	    var result = $.parseJSON(data);
	    if(result['status'] == 'success') {
  	...
  	$("#btn-submit").click(function(e) {
		if($('#customer-id').val() == '') {
		  ...
		  alert("Please Select Date");
		    ...
		    $.ajax({
		        type: "POST",
		        url: burl + "invoice/edit/" + $("#hid-quo-id").val(), 
		        data: { 
