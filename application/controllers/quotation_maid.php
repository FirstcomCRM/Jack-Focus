...
class quotation_maid extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model('quotation_maid_model');
		...
		$b_url = base_url().'quotation_maid/index';
		$t_rows = $this->quotation_maid_model->count();
		$pageConfig = create_pagination_config( $b_url, $t_rows, 10, 3);
		$this->pagination->initialize($pageConfig);
		$page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
		
		$data['quotations'] = $this->quotation_maid_model->fetch($pageConfig['per_page'], $page);
        ...
        $this->load->view('quotation_maid/index', $data);
	...
	public function add(){
		...
		if($this->form_validation->run() === FALSE) {
			...
			$this->load->view('_template/header', $data);
			$this->load->view('quotation_maid/add_edit', $data);
			$this->load->view('_template/footer', $data);
		}
		else{
			$quotation_id = $this->quotation_maid_model->add();
	...
	public function check_add_quotation_no(){
        ...
        $result = $this->quotation_maid_model->quotation_no_exist($quotation_no);
        if($result){
	...
	public function view($id){
		$data['msg'] = $this->session->flashdata('msg');
		$data['quotation'] = $this->quotation_maid_model->get($id);
		if (empty($data['quotation'])) {
			show_404();
		}
		$data['action'] = 'view';
		$data['customers'] = $this->customer_maid_model->get();
		$data['quotation_products'] = $this->quotation_maid_model->get_quotation_product($id);
		$data['products']  = $this->maid_model->get();
		$data['quotation_id']  = $id;
		$data['customer_info'] = $this->customer_maid_model->get($data['quotation']['customer_id']);
		$this->load->view('_template/header', $data);
		$this->load->view('quotation_maid/view', $data);
		$this->load->view('_template/footer', $data);	
	...
	public function edit($id){
		...
		if($this->form_validation->run() === FALSE) {
			...
			$this->quotation_maid_model->update($id);
	...
	public function check_edit_quotation_no($str, $id){
		$quotation_no = $this->input->post('quotation_no');
        $result = $this->quotation_maid_model->quotation_no_exist($quotation_no, $id);
        if($result){
	...
	public function aj_remove_quotation_product($id) {
		$this->quotation_maid_model->remove_quotation_product($id);
		$ret = array(
			'status' 	=> 'success',
		);
		echo json_encode($ret);
	...
	public function aj_edit_product_details($id){
		$this->quotation_maid_model->update_quotation_product($id);
		$ret = array(
			'status' 	=> 'success',
		);
		echo json_encode($ret);
	...
	public function print_preview($id){
		...
		$this->load->view('quotation_maid/print', $data);
	...
	public function export_quotation(){
		...
		$quotations = $this->quotation_maid_model->get();
		$quo_export =  array();
		$no = 1;
