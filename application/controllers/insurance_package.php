...
class insurance_package extends CI_Controller {
	...
	public function index() {
			...
			if($a['insu_view'] == 0){
				...
				$b_url = base_url().'insurance_package/index';
				$t_rows = $this->insurance_package_model->count();
				$pageConfig = create_pagination_config( $b_url, $t_rows, 10, 3);
				$this->pagination->initialize($pageConfig);
				$page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
				
				$data['insurancex'] = $this->insurance_package_model->fetch($pageConfig['per_page'], $page);
		        ...
		        $this->load->view('insurance_package/index', $data);
	...
	public function add(){
			...
			if($a['insu_add'] == 0){
				...
				$this->form_validation->set_rules('insurance_name', 'insurance_name', 'required|callback_check_add_company_name');
				...
				if($this->form_validation->run() === FALSE) {
					...
					$this->insurance_package_model->add();
