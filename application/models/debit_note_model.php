...
class debit_note_model extends CI_Model
{
    ...
    public function get($id=FALSE){
        if ($id === FALSE) {
            ...
            $query = $this->db->get('debit_maid_note');
            return $query->result_array();
        }
        $query = $this->db->get_where('debit_maid_note', array('debit_note_id' => $id, 'active' => 1));
    ...
    public function add() {
        ...
        $this->db->insert('debit_maid_note', $data);
    ...
    public function delete($id){
        ...
        return $this->db->update('debit_maid_note', $data);
    }

    public function count(){
        $this->db->select('dn.*, i.invoice_no, q.gst');
        $this->db->from('debit_note dn');
        $this->db->join('quotation_maid i', 'i.quotation_id=dn.quotation_id', 'left');
    ...
    public function fetch($limit, $start){
        $this->db->select('dn.*, i.invoice_no, q.gst');
        $this->db->from('debit_note dn');
        $this->db->join('quotation_maid i', 'i.invoice_id=dn.invoice_id', 'left');
        //$this->db->join('quotation q', 'q.quotation_id=dn.invoice_id', 'left');
        $this->db->where('dn.active', 1);
        if (isset($_GET['dn_no'])&&$_GET['dn_no']!='') {
            $this->db->like('dn.dn_no', $_GET['dn_no']);
    ...
    public function get_debit_note($quotation_id){
        $this->db->where('active =', 1);
    ...
    public function get_dn_total($quotation_id){
    ...
    public function dn_no_exist($dn_no, $id=FALSE){
        ...
        $query = $this->db->get_where('debit_maid_note', array('dn_no' => $dn_no));
        if ($query->num_rows()>=1) {
            return $query->row();
        }
        return false;
