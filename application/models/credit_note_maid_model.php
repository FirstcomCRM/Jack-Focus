...
* 
*/
class credit_note_maid_model extends CI_Model
{
    ...
    public function get($id=FALSE){
        ...
        $query = $this->db->get_where('credit_note', array('credit_note_id' => $id, 'active' => 1));
    ...
    public function count(){
        ...
        $this->db->join('invoice i', 'i.invoice_id=cn.invoice_id', 'left');
    ...
    public function get_cn_total($quotation_id){
        ...
        $this->db->where('active =', 1);
        $this->db->where('invoice_id =', $quotation_id;
        $this->db->limit(1);
        $query = $this->db->get('credit_note');
        $row = $query->row_array();
    ...
    public function cn_no_exist($cn_no, $id=FALSE){
        ...
        $query = $this->db->get_where('credit_note', array('cn_no' => $cn_no));
        if ($query->num_rows()>=1) {
            return $query->row();
        }
        return false;
